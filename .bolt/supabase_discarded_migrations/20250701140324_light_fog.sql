/*
  ...
  2. Security
    ...
    - Add policies for users to manage their own payment records
  
  3. Automation
    - Create trigger for updating timestamps
    - Create trigger for initializing payment records for new users
*/
...
-- Create policies for user_payments using DO blocks to check existence
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'user_payments' 
    AND policyname = 'Users can view their own payment status'
  ) THEN
    EXECUTE 'CREATE POLICY "Users can view their own payment status" 
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'user_payments' 
    AND policyname = 'Users can insert their own payment records'
  ) THEN
    EXECUTE 'CREATE POLICY "Users can insert their own payment records" 
      ON public.user_payments 
      FOR INSERT 
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'user_payments' 
    AND policyname = 'Users can update their own payment records'
  ) THEN
    EXECUTE 'CREATE POLICY "Users can update their own payment records" 
      ON public.user_payments 
      FOR UPDATE 
      USING (auth.uid() = user_id)';
  END IF;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_proc 
    WHERE proname = 'handle_new_user_payment'
  ) THEN
    EXECUTE 'CREATE OR REPLACE FUNCTION public.handle_new_user_payment()
    RETURNS TRIGGER 
    LANGUAGE plpgsql
    SECURITY DEFINER
