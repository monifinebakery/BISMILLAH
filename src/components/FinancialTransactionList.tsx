...
const FinancialTransactionList = ({
  ...
  // ✅ PERBAIKAN: Fungsi handleChange yang hilang sekarang sudah didefinisikan
  const handleChange = (field: keyof typeof formData, value: string | number) => {
    setFormData(prev => {
      const newData = { ...prev, [field]: value };
      // Jika tipe transaksi diubah, reset kategori karena daftarnya berbeda
      if (field === 'type') {
        newData.category = '';
      }
      return newData;
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-2">
                    <Badge className={transaction.type === 'pemasukan' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}>
                  ...
                  <div className="flex items-center gap-4 text-sm text-gray-600">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span className={`font-medium ${transaction.type === 'pemasukan' ? 'text-green-600' : 'text-red-600'}`}>{formatCurrency(transaction.amount)}</span>
                    </div>
                  </div>
                </div>
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4 py-4">
            <div>
              ...
              <Select value={formData.type} onValueChange={(value: 'pemasukan' | 'pengeluaran') => handleChange('type', value)}>
