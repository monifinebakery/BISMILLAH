...
/**
 * Hook for asset form validation
 */
export const useAssetValidation = ({
  ...
  // Validate before submit (comprehensive validation)
  const validateBeforeSubmit = useCallback(() => {
    const errors = validateEntireForm();
    
    // Additional business logic validations
    const enhancedErrors = { ...errors };
    ...
    if (formData.nilaiAwal && formData.nilaiSaatIni && 
      ...
      if (formData.nilaiSaatIni > formData.nilaiAwal) {
        enhancedErrors.nilaiSaatIni = 'Nilai sekarang tidak boleh lebih besar dari nilai awal';
      }
    }
    ...
    if (formData.tanggalPembelian instanceof Date) {
      ...
      if (formData.tanggalPembelian > today) {
        enhancedErrors.tanggalPembelian = 'Tanggal pembelian tidak boleh di masa depan';
      }
    }
    ...
    if (formData.depresiasi && typeof formData.depresiasi === 'number' && 
      ...
      const tolerance = formData.nilaiAwal * 0.05; // 5% tolerance
      
      if (Math.abs(formData.nilaiSaatIni - calculatedCurrentValue) > tolerance) {
        enhancedErrors.nilaiSaatIni = 'Nilai sekarang tidak sesuai dengan persentase depresiasi yang dimasukkan';
      }
    }

    return {
      isValid: isFormValid(enhancedErrors),
  ...
  const realtimeValidation = useMemo(() => {
    ...
    const rules: Record<keyof AssetFormData, (value: any) => string | undefined> = {
      nama: (value) => {
        if (!value || String(value).trim() === '') return 'Nama aset wajib diisi';
        if (String(value).length > 255) return 'Nama terlalu panjang';
        return undefined;
      },
      kategori: (value) => {
      ...
      nilaiAwal: (value) => {
        if (value === '' || value === null || value === undefined) return 'Nilai awal wajib diisi';
        if (isNaN(Number(value)) || Number(value) < 0) return 'Nilai awal harus berupa angka positif';
        return undefined;
      },
      nilaiSaatIni: (value) => {
        if (value === '' || value === null || value === undefined) return 'Nilai sekarang wajib diisi';
        if (isNaN(Number(value)) || Number(value) < 0) return 'Nilai sekarang harus berupa angka positif';
        return undefined;
      },
// src/components/assets/hooks/useAssetValidation.ts
