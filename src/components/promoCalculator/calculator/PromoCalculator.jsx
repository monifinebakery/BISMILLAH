...
// Lazy components - Simplified for actual components
const PromoTypeSelector = ({ selectedType, onTypeChange, onFormSubmit, isCalculating, recipes, isMobile }) => {
...
const PromoPreview = ({ type, data, onSave, isLoading, isMobile }) => {
...
// Mock data for demonstration (remove when using real hooks)
const mockRecipes = [
...
const PromoCalculator = () => {
  ...
  // Use real hooks when available, fallback to mock for demo
  const { recipes, isLoading: recipesLoading } = typeof useRecipe === 'function' ? useRecipe() : mockUseRecipe();
  const { calculatePromo, savePromo, isLoading: calculationLoading } = typeof usePromoCalculation === 'function' ? usePromoCalculation() : mockUsePromoCalculation();
