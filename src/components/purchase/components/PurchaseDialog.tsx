...
// Enhanced for Edit Mode + Smart "SimplePurchaseItemForm" add-item
...
const PurchaseDialog: React.FC<PurchaseDialogProps> = ({
  ...
  // Item management (we only use edit-related helpers here)
  const {
    showAddItem,
    setShowAddItem,
    editingItemIndex,
  ...
  // Reset states when dialog toggles
  useEffect(() => {
    if (isOpen) {
            ...
            <CardContent className="space-y-4">
              ...
              {canEdit && showAddItem && (
                <SimplePurchaseItemForm
                  ...
                  onAdd={(cleanData) => {
                    // pastikan subtotal terisi untuk UI
                    const subtotal =
                      Number(cleanData.kuantitas || 0) * Number(cleanData.hargaSatuan || 0);

                    // langsung gunakan addItem dari usePurchaseForm
                    addItem({
                      bahanBakuId: cleanData.bahanBakuId,
                      nama: cleanData.nama,
                      kuantitas: cleanData.kuantitas,
                      satuan: cleanData.satuan,
                      hargaSatuan: cleanData.hargaSatuan,
                      keterangan: cleanData.keterangan,
                      // info kemasan (biarkan undefined kalau kosong; transformer akan jadi null di DB)
                      ...(cleanData as any).jumlahKemasan !== undefined && {
                        jumlahKemasan: (cleanData as any).jumlahKemasan,
                      },
                      ...(cleanData as any).isiPerKemasan !== undefined && {
                        isiPerKemasan: (cleanData as any).isiPerKemasan,
                      },
                      ...(cleanData as any).satuanKemasan !== undefined && {
                        satuanKemasan: (cleanData as any).satuanKemasan,
                      },
                      ...(cleanData as any).hargaTotalBeliKemasan !== undefined && {
                        hargaTotalBeliKemasan: (cleanData as any).hargaTotalBeliKemasan,
                      },
                      // subtotal digunakan untuk tampilan; total purchase akan dihitung juga oleh hook
                      subtotal,
                    } as any);
                          ...
                          <div className="flex items-start justify-between">
                            <div className="flex-1 grid grid-cols-1 md:grid-cols-4 gap-4">
                              <div>
                                ...
                                {(item as any).jumlahKemasan > 0 &&
                                  (item as any).isiPerKemasan > 0 && (
                                    <div className="text-xs text-gray-500 mt-1">
                                      Kemasan: {(item as any).jumlahKemasan} ×{' '}
                                      {(item as any).isiPerKemasan} {item.satuan || 'unit'}
                                      {(item as any).satuanKemasan
                                        ? ` (${(item as any).satuanKemasan})`
                                        : ''}
                                    </div>
                                  )}
...
const EditItemForm: React.FC<{
  ...
  const handleSave = () => {
    onSave({
      ...editedItem,
      subtotal: (editedItem.kuantitas || 0) * (editedItem.hargaSatuan || 0),
    });
  };

  const subtotal = (editedItem.kuantitas || 0) * (editedItem.hargaSatuan || 0);
        ...
        <div className="text-sm text-blue-800">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <div>
                Qty: {item.kuantitas} {item.satuan}
              </div>
            ...
            <div>
              ...
              <div>
                Qty: {editedItem.kuantitas} {item.satuan}
              </div>
