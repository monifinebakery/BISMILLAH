...
export interface BahanBaku {
  ...
  // MODIFIED: Tambahkan properti baru
...
export const AppDataProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const addBahanBaku = async (bahan: Omit<BahanBaku, 'id'>) => {
    const newBahan: BahanBaku = {
      ...bahan,
      id: generateUUID(),
      // Pastikan userId, createdAt, updatedAt juga ada di newBahan untuk konsistensi lokal
      userId: (await supabase.auth.getSession()).data.session?.user.id,
      createdAt: new Date(),
      updatedAt: new Date(),
    };

    // MODIFIED: Transform to snake_case for Supabase insert, termasuk kolom baru
    const bahanToInsert = {
      id: newBahan.id,
      nama: newBahan.nama,
    ...
    setBahanBaku(prev => [...prev, newBahan]); // Update local state dengan camelCase
  ...
  const updateBahanBaku = async (id: string, updatedBahan: Partial<BahanBaku>) => {
    // MODIFIED: Transform to snake_case for Supabase update, termasuk kolom baru
    const bahanToUpdate: Partial<any> = {
      // properti yang ada di updatedBahan (camelCase)
    };

    // Mapping eksplisit untuk setiap properti yang mungkin diupdate
    ...
    // Properti yang perlu di-transform ke snake_case dan mungkin null
  ...
  const deleteBahanBaku = async (id: string) => {
    ...
    if (bahan) {
      addActivity({
        ...
        description: `Bahan ${bahan.nama} telah dihapus dari gudang`,
        type: 'stok',
      });
      toast.success(`Bahan ${bahan.nama} berhasil dihapus!`);
    }
    return true;
  };
  ...
  const deleteSupplier = async (id: string) => {
    ...
    if (supplier) {
      addActivity({
        ...
        description: `Supplier ${supplier.nama} telah dihapus`,
        type: 'supplier',
      });
  ...
  const updatePurchase = async (id: string, updatedPurchase: Partial<Purchase>) => {
    ...
    if (purchaseToUpdate.total_nilai) delete purchaseToUpdate.total_nilai; // Perbaiki: delete should use snake_case
    if (purchaseToUpdate.metode_perhitungan) delete purchaseToUpdate.metode_perhitungan; // Perbaiki: delete should use snake_case
    if (purchaseToUpdate.createdAt) delete purchaseToUpdate.createdAt; // createdAt should not be updated

    // Pastikan properti camelCase diubah menjadi snake_case sebelum update
    if (updatedPurchase.tanggal) purchaseToUpdate.tanggal = updatedPurchase.tanggal.toISOString();
    if (updatedPurchase.nomorPesanan) purchaseToUpdate.nomor_pesanan = updatedPurchase.nomorPesanan;
    if (updatedPurchase.namaPelanggan) purchaseToUpdate.nama_pelanggan = updatedPurchase.namaPelanggan;
    if (updatedPurchase.emailPelanggan) purchaseToUpdate.email_pelanggan = updatedPurchase.emailPelanggan;
    if (updatedPurchase.teleponPelanggan) purchaseToUpdate.telepon_pelanggan = updatedPurchase.teleponPelanggan;
    if (updatedPurchase.alamatPelanggan) purchaseToUpdate.alamat_pengiriman = updatedPurchase.alamatPelanggan;
    if (updatedPurchase.totalPesanan) purchaseToUpdate.total_pesanan = updatedPurchase.totalPesanan;
  ...
  const updateRecipe = async (id: string, updatedRecipe: Partial<Recipe>) => {
    const recipeToUpdate: Partial<any> = {
      ...updatedRecipe, // Spread updatedRecipe to include all fields
    ...
    // Hapus properti camelCase yang sudah ada padanan snake_case-nya
  ...
  const addHPPResult = async (result: Omit<HPPResult, 'id'>) => {
    const newResult: HPPResult = {
      ...result,
      id: generateUUID(),
      createdAt: new Date(), // Menambahkan createdAt
      updatedAt: new Date(), // Menambahkan updatedAt
    };
    ...
    const resultToInsert = {
      ...
      user_id: (await supabase.auth.getSession()).data.session?.user.id,
      created_at: newResult.createdAt?.toISOString(), // Menggunakan newResult.createdAt
      updated_at: newResult.updatedAt?.toISOString(), // Menggunakan newResult.updatedAt
    };
  ...
  const addOrder = async (order: NewOrder) => {
    const newOrder: Order = {
      ...order,
      id: generateUUID(),
      tanggal: new Date(), // Pastikan ini Date object
      createdAt: new Date(), // Tambahkan
      updatedAt: new Date(), // Tambahkan
  ...
  const addOrder = async (order: NewOrder) => {
    ...
    const orderToInsert = {
      ...
      user_id: (await supabase.auth.getSession()).data.session?.user.id,
      created_at: newOrder.createdAt?.toISOString(),
      updated_at: new Order().updatedAt?.toISOString(), // Fix: Should use newOrder.updatedAt
    };
  ...
  const addRecipe = async (recipe: Omit<Recipe, 'id'>) => {
    ...
    const recipeToInsert = {
      ...
      harga_jual_per_porsi: newRecipe.hargaJualPerPorsi,
      user_id: (await supabase.auth.getSession()).data.session?.user.id,
      created_at: newRecipe.createdAt?.toISOString(), // Menggunakan newRecipe.createdAt
      updated_at: newRecipe.updatedAt?.toISOString(), // Menggunakan newRecipe.updatedAt
    };
