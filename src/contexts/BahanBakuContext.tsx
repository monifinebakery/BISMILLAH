...
// COMPLETE VERSION - REAL-TIME FIX & NOTIFICATION INTEGRATION
...
interface BahanBakuContextType {
  ...
  getBahanBakuByName: (name: string) => BahanBaku | undefined; // Added back
  reduceStok: (name: string, quantity: number) => Promise<boolean>; // Added back
...
export const BahanBakuProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  // ✅ FIX: Simplified useEffect to prevent re-subscriptions.
  // The real-time channel setup now ONLY depends on the user.
  useEffect(() => {
    if (!user) {
      setBahanBaku([]);
  ...
  const addBahanBaku = async (bahanBakuData: Omit<BahanBaku, 'id' | 'userId' | 'createdAt' | 'updatedAt'>): Promise<boolean> => {
    ...
    try {
      ...
      // Real-time will handle the UI update by calling refreshData
      return true;
    } catch (error: any) {
