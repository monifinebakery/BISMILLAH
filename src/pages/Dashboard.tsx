...
// Helper function untuk format waktu (tidak berubah)
...
const Dashboard = () => {
    ...
    const filteredOrders = useMemo(() => {
        ...
        return orders.filter(order => {
            const orderDate = new Date(order.tanggal).getTime();
            return orderDate >= fromDate && orderDate <= toDate;
        });
    }, [orders, date]);

    const filteredActivities = useMemo(() => {
        if (!date?.from) return activities;
        const fromDate = new Date(date.from).setHours(0, 0, 0, 0);
        const toDate = date.to ? new Date(date.to).setHours(23, 59, 59, 999) : new Date(fromDate).setHours(23, 59, 59, 999);
        ...
        <div className="p-4 sm:p-6 space-y-6 bg-gray-50 min-h-screen">
                    ...
                    <CardContent className="p-4">
                        <div className="flex items-center">
                            ...
                            <div>
                                ...
                                <p className="font-semibold text-gray-800">{formatCurrency(revenueInRange)}</p>
                            </div>
                        </div>
                    </CardContent>
                </Card>
                    ...
                    <CardContent className="p-4">
                        <div className="flex items-center">
                            ...
                            <div>
                                ...
                                <p className="font-semibold text-gray-800">{ordersInRange}</p>
                            </div>
                        </div>
                    </CardContent>
                </Card>
                    ...
                    <CardContent className="p-4">
                        <div className="flex items-center">
                            ...
                            <div>
                                ...
                                <p className="font-semibold text-gray-800">{formatCurrency(profitInRange)}</p>
                    ...
                    <CardContent className="p-4">
                        <div className="flex items-center">
                            ...
                            <div>
                                ...
                                <p className="font-semibold text-orange-600">{outstandingInvoices}</p>
                            </div>
                        </div>
                    </CardContent>
                </Card>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                 <Card className="bg-white border border-gray-100 hover:shadow transition-shadow">
                     <Link to="/hpp" className="p-4 flex items-center h-full">
                 ...
                 <Card className="bg-white border border-gray-100 hover:shadow transition-shadow">
                     <Link to="/gudang" className="p-4 flex items-center h-full">
                 ...
                 <Card className="bg-white border border-gray-100 hover:shadow transition-shadow">
                     <Link to="/laporan" className="p-4 flex items-center h-full">
                         <div className="bg-purple-50 p-2 rounded-lg mr-3"><FileText className="h-5 w-5 text-purple-600" /></div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="space-y-6">
                    {/* Best Selling Products (sudah terfilter) */}
                    <Card className="bg-white border border-gray-100 shadow-sm">
                        <CardHeader>
                            <CardTitle className="flex items-center gap-2 text-gray-800"><Trophy className="h-5 w-5 text-yellow-500" /><span>Produk Terlaris</span></CardTitle>
                        </CardHeader>
                                    ...
                                    currentProducts.map((product, index) => (
                                        <div key={`${product.name}-${index}`} className="p-4 flex items-center hover:bg-gray-50">
                                            <div className="flex-shrink-0 w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center">
                                                <span className="text-sm font-medium text-gray-700">{productsStartIndex + index + 1}</span>
                                            </div>
                        ...
                        {bestSellingProducts.length > itemsPerPage && (
                             <CardFooter className="flex items-center justify-between py-3 border-t border-gray-100">
                                <Button variant="outline" size="sm" onClick={() => setProductsPage(productsPage - 1)} disabled={productsPage === 1}><ChevronLeft className="h-4 w-4" /></Button>
                                <span className="text-sm text-gray-500">Halaman {productsPage} dari {totalProductsPages}</span>
                                <Button variant="outline" size="sm" onClick={() => setProductsPage(productsPage + 1)} disabled={productsPage >= totalProductsPages}><ChevronRight className="h-4 w-4" /></Button>
                            </CardFooter>
                        )}
                    </Card>

                    {/* Critical Stock Alert (tidak terpengaruh filter) */}
                    <Card className="bg-white border border-gray-100 shadow-sm">
                        <CardHeader>
                            <CardTitle className="flex items-center gap-2 text-red-600"><Warehouse className="h-5 w-5" /><span>Stok Kritis</span></CardTitle>
                        </CardHeader>
                        <CardContent className="p-0">
                            <div className="divide-y divide-gray-100">
                                {bahanBaku.filter(item => item.stok <= item.minimum).length > 0 ? (
                                    bahanBaku.filter(item => item.stok <= item.minimum).slice(0, 5).map((item) => (
                                        <div key={item.id} className="p-4 hover:bg-gray-50">
                                            <p className="font-medium text-gray-800 truncate">{item.nama}</p>
                                            <div className="flex justify-between mt-1">
                                                <p className="text-sm text-gray-500">Stok: {item.stok} {item.satuan}</p>
                                                <p className="text-sm text-red-600 font-medium">Minimum: {item.minimum} {item.satuan}</p>
                                            </div>
                                        </div>
                                    ))
                                ) : (
                ...
                <div className="space-y-6">
                    ...
                    <Card className="bg-white border border-gray-100 shadow-sm">
                        <CardHeader>
                            <CardTitle className="flex items-center gap-2 text-gray-800"><Activity className="h-5 w-5 text-gray-600" /><span>Aktivitas Terbaru</span></CardTitle>
                        </CardHeader>
                        <CardContent className="p-0">
                            <Table>
                                <TableHeader>
                                ...
                                <TableBody>
                                    {activitiesLoading ? (
                                        <TableRow><TableCell colSpan={3} className="text-center text-gray-500 py-6">Memuat aktivitas...</TableCell></TableRow>
                                        ...
                                        currentActivities.map((activity) => {
                                            ...
                                            return (
                                                <TableRow key={activity.id}>
                                                    ...
                                                    <TableCell className="text-xs text-gray-500">{formatDateTime(activity.timestamp)}</TableCell>
                        ...
                        {filteredActivities.length > itemsPerPage && (
                            <CardFooter className="flex items-center justify-between py-3 border-t border-gray-100">
                                <Button variant="outline" size="sm" onClick={() => setActivitiesPage(activitiesPage - 1)} disabled={activitiesPage === 1}><ChevronLeft className="h-4 w-4" /></Button>
                                <span className="text-sm text-gray-500">Halaman {activitiesPage} dari {totalActivitiesPages}</span>
                                <Button variant="outline" size="sm" onClick={() => setActivitiesPage(activitiesPage + 1)} disabled={activitiesPage >= totalActivitiesPages}><ChevronRight className="h-4 w-4" /></Button>
                            </CardFooter>
                        )}
                    </Card>
                    
                    {/* Worst Selling Products (sudah terfilter) */}
                    <Card className="bg-white border border-gray-100 shadow-sm">
                         <CardHeader>
                             <CardTitle className="flex items-center gap-2 text-gray-800"><TrendingDown className="h-5 w-5 text-gray-600" /><span>Produk Kurang Laris</span></CardTitle>
                         </CardHeader>
                         <CardContent className="p-0">
                             <div className="divide-y divide-gray-100">
                                 {worstSellingProducts.length > 0 ? (
                                     worstSellingProducts.map((product, index) => (
                                         <div key={`${product.name}-${index}`} className="p-4 hover:bg-gray-50">
                                             <p className="font-medium text-gray-800 truncate">{product.name}</p>
