...
import { ChevronLeft, ChevronRight, ShoppingCart, Package, DollarSign, TrendingUp, Receipt, Bell, Activity as ActivityIcon, Users, Scale, Trophy, TrendingDown, CircleDollarSign, ListChecks, Calculator, Clock, Truck } from 'lucide-react';
...
const Dashboard = () => {
  ...
  // ===================================================================
  // --- Metrik KPI gabungan (semua perhitungan di sini) ---
  // ===================================================================
  const { 
    ...
    const validHpps = hppResults.map(r => r.hppPerPorsi).filter(hpp => hpp !== undefined && hpp !== null);
    ...
    const ordersWithOutstandingPayment = orders.filter(order => 
      order.status !== 'delivered' && order.status !== 'cancelled' && (order.totalPesanan || 0) > 0 // Hanya order aktif & punya total
    );
    const totalPiutangAmount = ordersWithOutstandingPayment.reduce((sum, order) => sum + (order.totalPesanan || 0), 0);

    const ordersOverdue = orders.filter(order =>
      (order.status === 'pending' || order.status === 'confirmed' || order.status === 'processing') && 
      order.tanggal && order.tanggal < startOfToday // Jatuh tempo kalau tanggal order lebih lama dari hari ini
    );
    const piutangJatuhTempoAmount = ordersOverdue.reduce((sum, order) => sum + (order.totalPesanan || 0), 0);
    const countPiutang = ordersOverdue.length;


    return { 
      totalProducts, totalBahanBakuUnit, averageHPP: avgHPP, lowStockCount: lowStock,
      totalRevenueToday: totalRevenue, totalExpenseToday: totalExpense, netProfitToday: netProfit, ordersToProcess,
      totalPiutang: totalPiutangAmount, piutangJatuhTempo: piutangJatuhTempoAmount, countPiutang: countPiutang
    };
  }, [orders, bahanBaku, financialTransactions, hppResults, today, recipes, startOfToday, endOfToday]);
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6">
      ...
      <div className="grid gap-4 grid-cols-2 md:grid-cols-3 lg:grid-cols-6">
        ...
        <Card><CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium">Omzet Hari Ini</CardTitle><DollarSign className="h-4 w-4 text-muted-foreground" /></CardHeader><CardContent><div className="text-2xl font-bold text-green-600">{formatCurrency(totalRevenueToday)}</div><p className="text-xs text-muted-foreground">Termasuk Pemasukan saja</p></CardContent></Card>
        <Card><CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium">Laba Bersih Hari Ini</CardTitle><TrendingUp className="h-4 w-4 text-muted-foreground" /></CardHeader><CardContent><div className="text-2xl font-bold text-blue-600">{formatCurrency(netProfitToday)}</div><p className="text-xs text-muted-foreground">Omzet - Pengeluaran Hari Ini</p></CardContent></Card>
        <Card><CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium">Piutang Belum Lunas</CardTitle><ListChecks className="h-4 w-4 text-muted-foreground" /></CardHeader><CardContent><div className="text-2xl font-bold text-red-600">{formatCurrency(piutangJatuhTempo)}</div><p className="text-xs text-muted-foreground">{countPiutang} order</p></CardContent></Card>
