...
import { usePurchase } from '@/contexts/PurchaseContext'; // Menggunakan hook konteks yang baru
import { useSupplier } from '@/contexts/SupplierContext'; // Menggunakan hook konteks yang baru
import { useBahanBaku } from '@/contexts/BahanBakuContext'; // <-- MENGGANTIKAN useAppData
...
const PurchaseManagement = () => {
  ...
  const handleAddItem = () => {
    if (!newItem.namaBarang || newItem.jumlah <= 0 || newItem.hargaSatuan < 0) {
      toast.error('Nama, kuantitas (>0), dan harga satuan wajib diisi.');
      return;
    }
    const item = {
      id: generateUUID(),
