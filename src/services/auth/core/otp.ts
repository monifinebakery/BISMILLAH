...
export const sendEmailOtp = async (
  ...
  try {
    ...
    logger.api("/auth/otp", "Sending OTP to:", {
      email,
      allowSignup,
      skipCaptcha,
    });
    ...
    if (error) {
      ...
      if (error.status === 500 || error.message?.includes("500")) {
        ...
        toast.error(
          "Server error: Periksa konfigurasi Supabase (CAPTCHA harus dinonaktifkan)",
        );
        return false;
      }
      ...
      if (
        ...
        toast.error(
          "Terlalu banyak permintaan. Tunggu beberapa menit sebelum mencoba lagi.",
        );
        return false;
      }
...
export const verifyEmailOtp = async (
  ...
  try {
    ...
    if (data.session && data.user) {
      logger.success("🎉 OTP verified successfully - Session created:", {
        userId: data.user.id,
      ...
      logger.info(
        "🔄 [OTP] Session created, waiting for AuthContext to detect change...",
      );

      // ✅ ENHANCED: Monitor for auth state change detection
      let authStateDetected = false;
      const checkAuthState = () => {
        try {
          const authEvent = new CustomEvent("otp-session-created", {
            detail: { userId: data.user.id, email: data.user.email },
          });
          window.dispatchEvent(authEvent);
          authStateDetected = true;
          logger.success("🚀 [OTP] Auth state change event dispatched");
        } catch (error) {
          logger.warn("Failed to dispatch auth state change event:", error);
        }
      };

      // Dispatch immediately and after small delay
      checkAuthState();
      ...
      logger.success("✅ [OTP] Session ready for redirect, returning true");
      ...
      toast.error(
        "Verifikasi berhasil tetapi sesi tidak dibuat. Silakan coba login ulang.",
      );
      return false;
    }
  } catch (error) {
