...
export const requireAdmin = async (req)=>{
  try {
    ...
    const { data, error } = await supabase.from("user_settings").select("role").eq("user_id", user.id).single();
...
export const requirePayment = async (req)=>{
  try {
    // First check authentication
    const authResult = await requireAuth(req);
    if (authResult instanceof Response) {
    ...
    const { data, error } = await supabase.from("user_payments").select("is_paid").eq("user_id", user.id).single();
