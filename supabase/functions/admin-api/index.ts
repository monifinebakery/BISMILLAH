    ...
    if (req.method === "GET") {
      ...
      if (path === "stats") {
        ...
        const [usersResult, paymentsResult, totalPaidResult] = await Promise.all([
          supabase.from("user_settings").select("*", { count: "exact" }),
          supabase.from("user_payments").select("*", { count: "exact" }),
          supabase.from("user_payments").select("amount").eq("is_paid", true),
        ]);
        
        const totalRevenue = totalPaidResult.data?.reduce((sum, payment) => 
