  ...
  try {
    ...
    try {
      ...
      if (bahanBaku && bahanBaku.length > 0) {
        const bahanBakuData = bahanBaku.map((item: any) => ({
          id: item.id,
          user_id: user.id,
          nama: item.nama,
        ...
        const { error: bahanBakuError } = await supabase
          .from('bahan_baku')
          .upsert(bahanBakuData, { onConflict: 'id' })
      ...
      if (suppliers && suppliers.length > 0) {
        const suppliersData = suppliers.map((item: any) => ({
          id: item.id,
          user_id: user.id,
          nama: item.nama,
          kontak: item.kontak,
      ...
      if (purchases && purchases.length > 0) {
        const purchasesData = purchases.map((item: any) => ({
          id: item.id,
          user_id: user.id,
        ...
        const { error: purchasesError } = await supabase
          .from('purchases')
          .upsert(purchasesData, { onConflict: 'id' })
      ...
      if (recipes && recipes.length > 0) {
        const recipesData = recipes.map((item: any) => ({
          id: item.id,
          user_id: user.id,
        ...
        const { error: recipesError } = await supabase
          .from('hpp_recipes')
          .upsert(recipesData, { onConflict: 'id' })
      ...
      if (hppResults && hppResults.length > 0) {
        const hppResultsData = hppResults.map((item: any) => ({
          id: item.id,
          user_id: user.id,
          nama: item.nama,
          ingredients: item.ingredients || [],
        ...
        const { error: hppResultsError } = await supabase
          .from('hpp_results')
          .upsert(hppResultsData, { onConflict: 'id' })
      ...
      if (activities && activities.length > 0) {
        const activitiesData = activities.map((item: any) => ({
          id: item.id,
          user_id: user.id,
          title: item.title,
      ...
      return new Response(
        JSON.stringify({ error: 'Sync failed: ' + syncError.message }),
