-- 20250909161034_complete_order_workflow.sql
-- Purpose: Provide secure order completion workflow (with stock deduction, financial sync, and COGS tracking)
-- Includes:
--   - get_recipe_ingredients_for_order(order_id uuid) RETURNS TABLE
--   - can_complete_order(order_id uuid) RETURNS JSON
--   - complete_order_and_deduct_stock(order_id uuid) RETURNS JSON (SECURITY DEFINER + auth.uid() checks)
--   - reverse_order_completion(order_id uuid) RETURNS JSON (SECURITY DEFINER + auth.uid() checks)
-- Grants EXECUTE to role `authenticated`
-- Adds safe indexes IF NOT EXISTS

-- Ensure search_path safety for SECURITY DEFINER routines
-- All functions are defined under public schema

BEGIN;

-- Drop existing functions to ensure clean re-create
...
-- Helper: Resolve recipe ingredients and stock requirement per order
...
DECLARE
  v_owner UUID;
BEGIN
  -- Ownership check: only owner can query the breakdown
  ...
  recipe_ingredients AS (
    ...
    JOIN public.hpp_recipes hr ON hr.id = ri.recipe_id
  ),
...
-- Pre-check: Can an order be completed (by stock availability)?
...
-- Main: Complete order with stock deduction, financial sync, and COGS tracking
...
BEGIN
  ...
  INSERT INTO public.financial_transactions (
    ...
    format('Pesanan %s', v_order.nomor_pesanan),
    v_completion_date,
  ...
  VALUES (
    ...
    format('Pemasukan Rp %s', TRIM(TO_CHAR(v_total_amount, '999,999,999,999.00'))),
    'keuangan', NULL, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
  );
  ...
  VALUES (
    gen_random_uuid(), v_user_id,
    'COGS Dicatat',
    format('Pemakaian bahan untuk pesanan #%s dicatat (%s item)', v_order.nomor_pesanan, v_pemakaian_items::TEXT),
    'profit', NULL, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
  );
...
-- Reverse completion: restore stock and mark financial tx as reversed
